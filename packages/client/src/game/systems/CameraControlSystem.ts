  ...
  return (world: World) => {
    defineAction({
      ...
      callback: (event) => {
        ...
        const { x, y } = camera.getWorldPoint(mouseEvent.x - left, mouseEvent.y - top);
    ...
    scene.input.on('wheel', (pointer: any, gameObjects: any, deltaX: number, deltaY: number, deltaZ: number) => {
